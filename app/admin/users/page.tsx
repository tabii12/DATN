...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[11px] font-semibold px-2.5 py-1 rounded-full ${map[status] ?? map.inactive}`}>
      <span className={`w-1.5 h-1.5 rounded-full inline-block ${dot[status] ?? dot.inactive}`} />
      {label[status] ?? status}
    </span>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Tổng {users.length} tài khoản</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Tổng tài khoản", value: users.length, icon: "👥", color: "text-blue-600 bg-blue-50" },
          { label: "Đang hoạt động", value: totalActive, icon: "✅", color: "text-emerald-600 bg-emerald-50" },
          { label: "Quản trị viên", value: totalAdmin, icon: "👑", color: "text-indigo-600 bg-indigo-50" },
          { label: "Bị khóa", value: totalBanned, icon: "🚫", color: "text-red-600 bg-red-50" },
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 mb-4 flex flex-wrap gap-3 items-center">
        ...
        {(search || filterStatus !== "all" || filterRole !== "all") && (
          <button
            onClick={() => { setSearch(""); setFilterStatus("all"); setFilterRole("all"); setCurrentPage(1); }}
              ...
              {paginated.map(user => (
                <tr key={user._id} className="hover:bg-gray-50/60 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    {user.isVerified
                      ? <span className="text-emerald-500 text-base" title="Đã xác thực">✔</span>
                      : <span className="text-gray-300 text-base" title="Chưa xác thực">✖</span>
                    }
                  </td>
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <div className="flex items-center gap-3 mb-5 p-3 bg-gray-50 rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-gray-800">{editingUser.email}</p>
                <p className="text-xs text-gray-400">ID: {editingUser._id.slice(-10)}</p>
              </div>
            </div>

            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5">Tên hiển thị</label>
                <input
                  value={editName}
      ...
      {deletingUser && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 text-center">
            ...
            <h2 className="text-lg font-bold text-gray-900 mb-2">Xóa người dùng?</h2>
            <p className="text-sm text-gray-500 mb-1">
              Bạn có chắc muốn xóa tài khoản
            </p>
            <p className="font-semibold text-gray-800 mb-5">"{deletingUser.name}"?</p>
