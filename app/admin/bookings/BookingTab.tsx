...
interface BookingRaw {
  ...
  total: number;
  singleRooms: number;
  status: "paid_50" | "paid_100" | "cancelled";
  createdAt: any;
  orderId: string;
  vnpay?: {
    amount?: number;
...
<div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          ...
          <div className="flex gap-1.5 bg-white border border-gray-200 rounded-xl p-1 shadow-sm overflow-x-auto">
            {["all", "paid_50", "paid_100", "cancelled"].map((f) => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${
                  filterStatus === f ? "bg-orange-500 text-white" : "text-gray-500 hover:bg-gray-50"
                }`}
              >
                {f === "all" ? "Tất cả"
                  : f === "paid_50" ? "Đã TT 50%"
                  : f === "paid_100" ? "Đã TT 100%"
                  : "Đã hủy"}
              </button>
            ))}
          </div>
                    ...
                    <div className="px-5 pb-5 animate-in fade-in slide-in-from-top-1 duration-200">
                      <div className="bg-white rounded-xl border border-gray-100 p-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-xs shadow-inner">
                        ...
                        <div className="flex flex-col justify-center gap-1">
                          ...
                          <code className="text-[10px] bg-gray-100 p-1 rounded text-gray-600">{b.id}</code>
                        </div>
                      </div>
                    </div>
                  )}
