...
export default function AdminSales() {
  ...
  const openEdit = (tour: Tour, sale: Sale) => {
    setSelectedTour(tour);
    setEditingSale(sale);
  ...
  const handleSave = async () => {
    ...
    if (isNaN(pct) || pct <= 0 || pct >= 100) {
      alert("Discount phải từ 1 đến 99%");
      return;
    }
    setSaving(true);
    try {
      if (modal === "create") {
        const res = await fetch(`${API_SALES}/create`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ tour_id: selectedTour._id, discount: pct }),
        });
        const data = await res.json();
        if (!res.ok) { alert(data.message || "Tạo thất bại"); return; }
      } else if (modal === "edit" && editingSale) {
        const res = await fetch(`${API_SALES}/${editingSale._id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const filtered = tours.filter((t) =>
    ...
    (t.hotel_id?.city || "").toLowerCase().includes(search.toLowerCase())
  );
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div>
          ...
          <p className="text-[11px] text-gray-400 mt-0.5">
            {sales.length} tour đang giảm giá / {tours.length} tour
          </p>
        </div>
      </div>
                    ...
                    <div key={tour._id} className="bg-white rounded-xl border border-orange-100 p-4 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {tour.hotel_id?.city && (
                            <span className="text-[11px] text-gray-400">{tour.hotel_id.city}</span>
                          )}
                          {tour.category_id?.name && (
                            <span className="text-[11px] bg-orange-50 text-orange-400 px-2 py-0.5 rounded-full font-semibold">
                              {tour.category_id.name}
                            </span>
                          )}
                        </div>
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        <div className="bg-orange-500 text-white text-sm font-black px-4 py-2 rounded-xl">
                          -{sale.discount}%
                        </div>
                        <button onClick={() => openEdit(tour, sale)}
                          className="text-xs font-semibold text-blue-500 hover:bg-blue-50 px-2.5 py-2 rounded-lg border-none cursor-pointer bg-transparent transition-colors">
                          Sửa
                        </button>
                        <button onClick={() => handleDelete(sale, tour.name)}
                          className="text-xs font-semibold text-red-400 hover:bg-red-50 px-2.5 py-2 rounded-lg border-none cursor-pointer bg-transparent transition-colors">
                          Xoá
                        </button>
                      </div>
                    </div>
                  ...
                  <div key={tour._id} className="bg-white rounded-xl border border-gray-100 p-4 flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {tour.category_id?.name && (
                          <span className="text-[11px] bg-gray-50 text-gray-400 px-2 py-0.5 rounded-full font-semibold">
                            {tour.category_id.name}
                          </span>
                        )}
                      </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100">
              <h2 className="text-base font-black text-gray-900">
                {modal === "create" ? "🏷️ Tạo sale" : "✏️ Sửa sale"}
              </h2>
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="flex items-center gap-3 bg-gray-50 rounded-xl p-3">
                ...
                <div className="min-w-0">
                  ...
                  {selectedTour.hotel_id?.city && (
                    <p className="text-[11px] text-gray-400">{selectedTour.hotel_id.city}</p>
                  )}
                </div>
              </div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {[5, 10, 15, 20, 25, 30, 50].map((v) => (
                    <button key={v} type="button" onClick={() => setDiscount(String(v))}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold border-2 cursor-pointer transition-all ${
                        discount === String(v)
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3">
              ...
              <button onClick={handleSave} disabled={saving || !discount}
                className={`flex-1 py-2.5 rounded-xl text-sm font-bold border-none cursor-pointer transition-colors text-white ${
                  saving || !discount ? "bg-orange-300 cursor-not-allowed" : "bg-orange-500 hover:bg-orange-600"
                }`}>
                {saving ? (
