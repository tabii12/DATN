...
export default function CreateTourPage() {
  ...
  const [form, setForm] = useState({
    name: "", status: "active", category_id: "", start_location: "", duration: 1,
  });
              ...
              <div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <input type="number" min={1} max={30} value={form.duration}
                    ...
                    className="w-24 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all text-center font-bold"/>
                  <span className="text-sm text-gray-400">
                    {form.duration} ngày {form.duration > 1 ? `${form.duration - 1} đêm` : ""}
                  </span>
                  <div className="flex gap-1.5 ml-auto">
                    {[2, 3, 4, 5, 7].map(d => (
                      <button key={d} type="button"
                        onClick={() => setForm(f => ({ ...f, duration: d }))}
                        className={`px-3 py-1.5 rounded-lg text-xs font-bold border cursor-pointer transition-colors ${form.duration === d ? "bg-orange-500 text-white border-orange-500" : "bg-gray-50 text-gray-500 border-gray-200 hover:border-orange-300"}`}>
                        {d}N
                      </button>
                    ))}
                  </div>
