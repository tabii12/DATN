...
function InlineField({
  ...
  const displayVal = renderDisplay ? (
    ...
    <span
      className={
        mono ? "font-mono text-gray-400 text-xs" : "text-sm text-gray-800"
      }
    >
      {value}
    </span>
      ...
      {!editing ? (
        <div className="flex items-center gap-3">
          ...
          {!readOnly && (
            ...
            >
              <svg
                className="w-3.5 h-3.5 text-gray-400 hover:text-orange-500"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
...
function DurationSetForm({
  onSave,
}: {
  onSave: (days: number, nights: number) => Promise<void>;
}) {
...
export default function TourInfo({
  ...
  useEffect(() => {
    fetch(`${API}/services/all`)
      .then((r) => r.json())
      .then((d) =>
        setHotels(d.data?.filter((s: any) => s.type === "hotel") || []),
      );
  }, []);
  ...
  const createHotel = async () => {
    ...
    try {
      const res = await fetch(`${API}/services/create`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          serviceName: hotelForm.name,
          address: hotelForm.address,
          city: hotelForm.city,
          description: hotelForm.description,
          basePrice: Number(hotelForm.price_per_night),
      ...
      const newHotels = await fetch(`${API}/services/all`)
        .then((r) => r.json())
        .then((d) => d.data?.filter((s: any) => s.type === "hotel") || []);
      setHotels(newHotels);
    ...
    <div className="space-y-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button
                onClick={() => setShowHotelModal(false)}
                className="w-8 h-8 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-gray-50 bg-white cursor-pointer text-lg"
              >
                ✕
              </button>
            </div>
            <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
              <div>
                ...
                <input
                  ...
                  className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-orange-400 transition-all"
                />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <InlineField
          ...
          renderDisplay={(val) => (
            <span
              className={`text-xs font-bold px-2.5 py-1 rounded-full ${val === "active" ? "bg-emerald-50 text-emerald-600" : "bg-gray-100 text-gray-400"}`}
            >
            ...
            <div className="grid grid-cols-2 gap-3">
              {(
                ...
                >
                  ...
                  <p
                    className={`text-xs font-bold mt-1.5 ${val === v ? "text-orange-700" : "text-gray-700"}`}
                  >
                    {l}
                  </p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        {!tour.duration ? (
          <DurationSetForm
            onSave={async (d, n) => {
              await saveField({ duration: d, nights: n } as any);
            }}
          />
        ) : (
          <InlineField
            label="Số ngày"
            value={String(tour.duration)}
            icon="📅"
            onSave={async (val) => {
              const n = Number(val);
            ...
            renderDisplay={(val) => (
              <span className="text-sm font-bold text-gray-800">
                {val} ngày {(tour as any).nights ?? Number(val) - 1} đêm
              </span>
            )}
            renderEdit={(val, set) => (
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <InlineField
          ...
          renderDisplay={() =>
            tour.category_id ? (
              <span className="text-sm font-semibold text-orange-600 bg-orange-50 px-2.5 py-1 rounded-lg">
