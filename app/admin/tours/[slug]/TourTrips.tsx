...
// Tính min từ max: 80%, tối thiểu 1
const calcMin = (max: number) => Math.max(1, Math.round(max * 0.8));
...
export default function TourTrips({ tourId, trips, duration, onRefresh }: Props) {
  ...
  const [tripForm, setTripForm] = useState({
    start_date: "", end_date: "",
    min_people: calcMin(10), max_people: 10,
  ...
  // Khi thay đổi max_people → tự tính lại min_people = 80%
  const handleMaxChange = (maxStr: string) => {
    const max = Math.max(1, Number(maxStr));
    setTripForm(f => ({ ...f, max_people: max, min_people: calcMin(max) }));
  };
  ...
  const handleEditClick = (trip: Trip) => {
    ...
    setTripForm({
      ...
      min_people:  trip.min_people  ?? calcMin(trip.max_people),
      max_people:  trip.max_people,
      selected_services: services
        .map(s => ({
          service_id: typeof s.service_id === "string" ? s.service_id : (s.service_id as any)?._id ?? "",
          quantity: s.quantity,
          note: s.note || "",
        }))
  ...
  const resetForm = () => {
    ...
    setTripForm({ start_date: "", end_date: "", min_people: calcMin(10), max_people: 10, selected_services: [] });
  };
  ...
  const deleteTrip = async (id: string) => {
    ...
    try {
      ...
      if (res.ok) { setLocalTrips(prev => prev.filter(t => t._id !== id)); }
      else alert("Lỗi xoá: " + (data.message || "Không thể xoá"));
    } catch { alert("Lỗi server khi xoá chuyến đi"); }
  };
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
          ...
          <div>
            ...
            {tripForm.end_date && <p className="text-[10px] text-orange-500 mt-1 font-semibold">Tự tính từ {duration} ngày tour</p>}
          </div>

          {/* Tối đa — đổi trước để auto-fill min */}
          <div>
          ...
          {/* Tối thiểu — tự động 80%, admin vẫn có thể chỉnh tay */}
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-1">
              Tối thiểu (người)
              <span className="ml-1.5 text-[10px] font-normal text-orange-400 bg-orange-50 px-1.5 py-0.5 rounded-full">
                tự động 80%
              </span>
            </label>
            <input type="number" min={1} max={tripForm.max_people} value={tripForm.min_people}
              onChange={e => setTripForm(f => ({ ...f, min_people: Math.max(1, Math.min(f.max_people, Number(e.target.value))) }))}
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-orange-400"/>
            <p className="text-[10px] text-gray-400 mt-1">
              = {Math.round(tripForm.min_people / tripForm.max_people * 100)}% tổng chỗ
            </p>
          </div>
        </div>
        ...
        <div className="mb-4">
          ...
          {globalServices.length === 0 ? (
            <div className="py-8 text-center border-2 border-dashed border-gray-100 rounded-xl">
              <p className="text-xs text-gray-400">Không tìm thấy dịch vụ nào.</p>
            </div>
          ) : (
        ...
        <button onClick={handleSubmitTrip} disabled={loading}
          ...
          {loading
            ? <span className="flex items-center justify-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/>Đang xử lý...</span>
            : editingTripId ? "💾 Lưu cập nhật" : "+ Thêm chuyến"}
        </button>
      </div>
      ...
      {localTrips.length === 0 ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          ...
          <div className="divide-y divide-gray-50">
            {[...localTrips.filter(t => t.status !== "deleted")]
              ...
              .map(trip => {
                ...
                const minPeople = trip.min_people ?? calcMin(trip.max_people);
