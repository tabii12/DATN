...
type Itinerary = { _id: string; day_number: number; title: string; meal_note: string; details: ItineraryDetail[]; };
...
type Tour = {
  ...
  hotel_id?: Hotel; category_id?: Category;
  images: TourImage[]; descriptions: TourDescription[]; itineraries: Itinerary[];
};
...
export default function EditTourPage() {
  ...
  useEffect(() => {
    if (!slug) return;
    fetchTour();
    fetch(`${API}/hotels`).then(r => r.json()).then(d => setHotels(d.data || []));
  ...
  const updateInfo = async () => {
    ...
    await fetch(`${API}/tours/detail/${slug}`, {
      ...
      body: JSON.stringify({ name: tour.name, status: tour.status, hotel_id: tour.hotel_id?._id, category_id: tour.category_id?._id }),
    });
    setSavingInfo(false);
    fetchTour();
  };
  ...
  const addDescription = async () => {
    ...
    setDescTitle(""); setDescContent(""); fetchTour();
  };
  ...
  const addDay = async () => {
    ...
    await fetch(`${API}/tours/${slug}/itineraries`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ day_number: nextDay, title: `Ngày ${nextDay}`, meal_note: "" }),
    });
    fetchTour();
  };
  ...
  const updateDetail = async (id: string) => {
    await fetch(`${API}/details/${id}`, {
      method: "PUT", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ title: editDetailTitle, content: editDetailContent, type: editDetailType }),
    });
    setEditingDetail(null); fetchTour();
  };

  const deleteDetail = async (id: string) => {
    if (!confirm("Xoá hoạt động này?")) return;
    await fetch(`${API}/details/${id}`, { method: "DELETE" }); fetchTour();
  };
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      <div className="bg-white border-b border-gray-100 px-6 py-4 flex items-center gap-3 sticky top-0 z-20">
        <button onClick={() => router.push("/admin/tours")}
          className="w-8 h-8 rounded-xl border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-50 bg-white cursor-pointer transition-colors text-lg">‹</button>
          ...
          {TABS.map(t => (
            <button key={t.key} onClick={() => setTab(t.key)}
              className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold whitespace-nowrap transition-all border-none cursor-pointer flex-1 justify-center ${
                tab === t.key ? "bg-orange-500 text-white shadow-sm" : "text-gray-500 bg-transparent hover:bg-gray-50"
              }`}>
              <span>{t.icon}</span>
        ...
        {tab === "info" && (
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-semibold text-gray-700 block mb-1.5">Tên tour</label>
                  <input className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all"
                    value={tour.name} onChange={e => setTour({ ...tour, name: e.target.value })} />
                </div>
                <div>
                  ...
                  <div className="flex items-center border border-gray-100 rounded-xl bg-gray-50 px-4 py-3">
                    ...
                    <span className="text-sm text-gray-400 font-mono ml-1">{tour.slug}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-semibold text-gray-700 block mb-1.5">Khách sạn</label>
                  <select className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-400 bg-white cursor-pointer"
                    value={tour.hotel_id?._id || ""}
                    onChange={e => setTour({ ...tour, hotel_id: hotels.find(h => h._id === e.target.value) })}>
                ...
                <div>
                  ...
                  <select className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-orange-400 bg-white cursor-pointer"
                    ...
                    onChange={e => setTour({ ...tour, category_id: categories.find(c => c._id === e.target.value) })}>
                    <option value="">-- Chọn --</option>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {([["active", "✅", "Hoạt động", "Hiển thị công khai"], ["inactive", "🚫", "Tạm ẩn", "Không hiển thị"]] as const).map(([v, icon, l, sub]) => (
                  <button key={v} type="button" onClick={() => setTour({ ...tour, status: v })}
                    className={`p-4 rounded-xl border-2 text-left cursor-pointer transition-all ${
                      tour.status === v ? "border-orange-400 bg-orange-50" : "border-gray-100 bg-gray-50 hover:border-gray-200"
                    }`}>
                    <span className="text-2xl">{icon}</span>
                    <p className={`text-sm font-bold mt-1.5 ${tour.status === v ? "text-orange-700" : "text-gray-700"}`}>{l}</p>
            ...
            <button onClick={updateInfo} disabled={savingInfo}
                  ...
                  <div key={img._id} className="group relative rounded-xl overflow-hidden bg-gray-100 aspect-video">
                    ...
                    <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                      <button onClick={() => deleteImage(img._id)}
                        className="bg-red-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg border-none cursor-pointer">Xoá</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                <input placeholder="Tiêu đề (VD: Giá tour bao gồm)" value={descTitle}
                ...
                <textarea placeholder="Nội dung chi tiết..." value={descContent}
                  onChange={e => setDescContent(e.target.value)} rows={5}
                  ...
                  <div key={d._id} className="bg-white rounded-2xl border border-gray-100 p-5 group">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <button onClick={() => deleteDescription(d._id)}
                        className="text-xs text-red-400 hover:text-red-600 border-none cursor-pointer bg-transparent opacity-0 group-hover:opacity-100 transition-opacity shrink-0">Xoá</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {tour.itineraries.sort((a, b) => a.day_number - b.day_number).map(day => (
              <div key={day._id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
                <div className="px-5 py-3.5 flex items-center justify-between gap-2 border-b border-gray-100 bg-gray-50/60">
                  {editingDay === day._id ? (
                    <div className="flex-1 flex flex-col gap-2">
                      <input value={editingDayTitle} onChange={e => setEditingDayTitle(e.target.value)}
                      ...
                      <input value={editingDayMeal} onChange={e => setEditingDayMeal(e.target.value)}
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <button onClick={() => { setEditingDay(day._id); setEditingDayTitle(day.title); setEditingDayMeal(day.meal_note); }}
                          className="text-xs font-semibold text-blue-500 hover:bg-blue-50 px-2.5 py-1.5 rounded-lg border-none cursor-pointer bg-transparent">Sửa</button>
                        <button onClick={() => deleteDay(day._id)}
                          className="text-xs font-semibold text-red-400 hover:bg-red-50 px-2.5 py-1.5 rounded-lg border-none cursor-pointer bg-transparent">Xoá</button>
                      </div>
                    </>
                  )}
                </div>
                ...
                <div className="p-4 space-y-2">
                      ...
                      <div key={dt._id} className="border border-gray-100 rounded-xl overflow-hidden group/detail">
                        {editingDetail === dt._id ? (
                          <div className="p-4 space-y-3 bg-orange-50/30">
                              ...
                              {Object.entries(TYPE_META).map(([v, m]) => (
                                <button key={v} type="button" onClick={() => setEditDetailType(v)}
                                  className={`flex items-center gap-1 text-xs font-bold px-3 py-1.5 rounded-xl border-2 cursor-pointer transition-all ${
                                    editDetailType === v ? "border-orange-400 bg-orange-50 text-orange-600" : "border-gray-100 bg-white text-gray-500"
                                  }`}>
                                  {m.icon} {m.label}
                                </button>
                              ))}
                            </div>
                            <input value={editDetailTitle} onChange={e => setEditDetailTitle(e.target.value)}
                            ...
                            <textarea value={editDetailContent} onChange={e => setEditDetailContent(e.target.value)} rows={3}
                              ...
                              {images.length > 0 && (
                                <div className="flex gap-1.5 mt-2">
                                  {images.slice(0, 3).map((img, k) => (
                                    <img key={k} src={img.image_url} className="h-12 w-16 object-cover rounded-lg" />
                                  ))}
                                </div>
                              )}
                            </div>
                            <div className="flex gap-1 shrink-0 opacity-0 group-hover/detail:opacity-100 transition-opacity">
                              <button onClick={() => { setEditingDetail(dt._id); setEditDetailTitle(displayTitle); setEditDetailContent(displayContent); setEditDetailType(dt.type); }}
                              ...
                              <button onClick={() => deleteDetail(dt._id)}
                  ...
                  <button onClick={() => addDetail(day._id)}
