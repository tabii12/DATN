...
function LatestNewsSection() {
  ...
  useEffect(() => {
    async function loadBlogs() {
      ...
      try {
        ...
        const sortedBlogs = items.sort((a: BlogItem, b: BlogItem) => new Date(b.createdAt || 0).getTime() - new Date(a.createdAt || 0).getTime());
  ...
  if (loading) {
    return <div className="text-center text-gray-500 py-10">Đang tải tin tức...</div>;
  }
  ...
  return (
    <div className="space-y-4">
        ...
        {blogs.slice(0, 4).map((blog) => (
          <div key={blog._id} className="bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
            ...
            <div className="p-4">
              ...
              <p className="text-gray-600 text-sm mt-2 line-clamp-3">{blog.excerpt ?? (blog.content ? `${blog.content.replace(/<[^>]*>/g, "").slice(0, 60
              )}...` : "")}</p>
              <div className="flex items-center justify-between mt-3 text-xs text-gray-500">
                <span>{blog.createdAt ? new Date(blog.createdAt).toLocaleDateString('vi-VN') : ''}</span>
                <a href={`/blogs/${blog.slug}`} className="text-orange-500 hover:text-orange-600 font-medium">Xem tiếp →</a>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="text-right">
        <a href="/blogs" className="text-sm font-semibold text-orange-500 hover:text-orange-600">Xem tất cả tin tức</a>
      </div>
    </div>
  );
}
