...
interface ItineraryDetail { place_id: PlaceDetail; }
interface Itinerary { day: number; title: string; details: ItineraryDetail[]; }
interface TourAPI {
                              ...
                              {place.images && place.images.length > 0 && (
                                <div className="flex gap-2 mt-2 overflow-x-auto pb-1">
                                  {place.images.slice(0, 4).map((img, k) => (
                                    <img key={k} src={img.image_url} alt={place.title} className="h-24 w-36 object-cover rounded-lg flex-shrink-0" loading="lazy" />
                                  ))}
                                </div>
                              )}
                            </div>
