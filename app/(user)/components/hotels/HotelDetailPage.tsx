...
interface TourAPI {
  ...
  hotel_id: { name: string; address: string; city: string; rating: number; price_per_night: number; };
...
function scoreFromRating(s: number) { return Math.min(9.9, parseFloat((s * 1.8 + 0.8).toFixed(1))); }
function scoreLabel(s: number) { return s >= 9.0 ? "Tuyệt vời" : s >= 8.5 ? "Rất tốt" : "Tốt"; }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-5">
      ...
      <div className="text-center">
        ...
        {attempt > 0 && (
          <p className="text-xs text-gray-400 mt-1">Thử lần {attempt}/{max} — Render server cần ~30-60s wake up</p>
        )}
      </div>
      {attempt > 0 && (
...
export default function HotelDetailPage({ slug }: { slug: string }) {
  ...
  function startFetch(n: number) {
    setAttempt(n);
    Promise.all([
      fetch(BASE_URL + "/api/tours").then(r => { if (!r.ok) throw new Error("bad"); return r.json(); }),
      fetch(BASE_URL + "/api/tours/detail/" + slug).then(r => { if (!r.ok) throw new Error("bad"); return r.json(); }),
    ])
          ...
          <div className="flex justify-between items-start py-4 gap-4">
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-1">
                ...
                {tour.category_id && (
                  <span className="text-[11px] bg-orange-100 text-orange-600 font-semibold px-2 py-0.5 rounded-full">{tour.category_id.name}</span>
                )}
              </div>
            ...
            <div className="max-w-[1200px] mx-auto px-4 pb-4">
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {subImgs.length > 0 && (
                    <div className="grid gap-1 mt-1 h-[110px]" style={{ gridTemplateColumns: "repeat(" + subImgs.length + ", 1fr)" }}>
                      {subImgs.map((img, i) => (
                    ...
                    <div className="flex-1 p-3 flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center gap-2">
                          <div className="w-7 h-7 rounded-full bg-orange-50 flex items-center justify-center text-[10px] font-bold text-orange-500 flex-shrink-0">TD</div>
              ...
              <div className="bg-white px-4 py-2 flex gap-1 overflow-x-auto">
                {thumbs.map((img, i) => (
                  <div key={i} onClick={() => setActiveImg(i)}
                    className={"flex-shrink-0 rounded cursor-pointer border-2 overflow-hidden transition-colors " + (activeImg === i ? "border-orange-500" : "border-transparent")}
                              ...
                              {images.length > 0 && (
                                <div className="flex gap-2 mt-2 overflow-x-auto pb-1">
                                  {images.slice(0, 4).map((img, k) => (
                                    <img key={k} src={img.image_url} alt={title} className="h-24 w-36 object-cover rounded-lg flex-shrink-0" loading="lazy" />
                                  ))}
                                </div>
                              )}
                            </div>
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-orange-400 to-amber-300 flex items-center justify-center text-white text-xs font-black shrink-0">
            {review.avatar}
          </div>
          <div>
      ...
      {review.tags.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-3">
          {review.tags.map(tag => (
            <span key={tag} className="text-[11px] bg-orange-50 text-orange-500 font-semibold px-2.5 py-0.5 rounded-full">
              ✓ {tag}
            </span>
          ))}
        </div>
      ...
      <div className="flex items-center gap-1 text-[11px] text-gray-400">
        <button onClick={() => onHelpful(review.id)}
          className="flex items-center gap-1 hover:text-orange-500 transition-colors bg-transparent border-none cursor-pointer text-[11px] text-gray-400 hover:text-orange-500 p-0">
...
function ReviewSection({ hotelName, tourId }: { hotelName: string; tourId: string }) {
  ...
  const sorted = [...reviews]
    .sort((a, b) => sortBy === "oldest" ? a.id.localeCompare(b.id) : b.id.localeCompare(a.id));
      ...
      <div className="max-w-[1200px] mx-auto px-4 py-6">
        ...
        <div className="flex flex-col sm:flex-row gap-5 mb-6 p-5 bg-gray-50 rounded-2xl">
          ...
          <div className="flex items-center gap-4 shrink-0">
            <div className={`text-4xl font-black text-white w-20 h-20 rounded-2xl flex items-center justify-center shadow-lg ${scoreColor}`}>
              {avgScore.toFixed(1)}
            </div>
            <div>
          ...
          <div className="mb-6 bg-orange-50/50 border border-orange-100 rounded-2xl p-5 space-y-4">
            ...
            <div>
              <label className="text-xs font-semibold text-gray-600 block mb-2">
                Thêm hình ảnh <span className="text-gray-400 font-normal">(tối đa 3 ảnh)</span>
              </label>
              <div className="flex flex-wrap gap-2">
          ...
          {([["newest", "Mới nhất"], ["oldest", "Cũ nhất"]] as const).map(([v, l]) => (
            <button key={v} onClick={() => setSortBy(v)}
              className={`text-xs font-semibold px-3 py-1.5 rounded-full border cursor-pointer transition-all ${
                sortBy === v ? "bg-orange-500 text-white border-orange-500" : "bg-white border-gray-200 text-gray-600 hover:border-orange-300"
              }`}>{l}</button>
          ))}
        </div>
        ...
        {sorted.length === 0 ? (
          <div className="py-12 text-center text-gray-400">
            ...
            <p className="font-semibold text-sm">Không có đánh giá nào phù hợp</p>
          </div>
        ) : (
