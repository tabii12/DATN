...
function mapTourToHotel(
  tour: TourAPI,
  allTrips: TripAPI[],
  commentMap: Record<string, { avg: number; count: number }>,
  ...
  const categoryName =
    (tour.category_id as any)?.name ?? categoryMap[rawCatId] ?? "Tour du lịch";
    ...
    score >= 9.0
      ...
      : score >= 8.5
        ...
        : score > 0
          ? "Tốt"
          : "Chưa có đánh giá";
  const comboDesc = tour.descriptions?.find(
    (d) => d.title === "Giá tour bao gồm",
  );
  const comboText =
    comboDesc?.content?.split("\n")?.[0]?.replace("- ", "") ?? null;
  ...
  const trips = allTrips.filter((t) => {
    const tid =
      typeof t.tour_id === "object" ? (t.tour_id as any)?._id : t.tour_id;
    return tid === tour._id;
  });

  const startLocation = (tour as any).start_location || "";

  const routeMatch = tour.name.match(
    /từ\s+([A-ZÀ-Ỹa-zà-ỹ\s.]+?)(?:\s*[-–]|$)/i,
  );
  const route = startLocation || (routeMatch ? routeMatch[1].trim() : "TP.HCM");
  if (
    startLocation.toLowerCase().includes("nội") ||
    startLocation.toLowerCase().includes("ha noi") ||
    startLocation.toLowerCase().includes("hà nội")
  ) {
  }
  return {
    id: tour._id,
    ...
    city: startLocation,
    tags: [startLocation, categoryName, `${hotelStars} sao`],
    stars: hotelStars,
    days,
    route,
    trips,
  ...
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <svg
          key={i}
          className={`w-3 h-3 ${i <= count ? "fill-amber-400" : "fill-gray-200"}`}
          viewBox="0 0 20 20"
        >
...
function RatingBadge({ rating }: { rating: number }) {
  ...
  return (
    <span
      className={`${color} text-white text-xs font-black px-1.5 py-0.5 rounded`}
    >
      {rating.toFixed(1)}
    </span>
  );
        ...
        <div className="flex flex-col gap-1">
          {nextTrip ? (
            <div className="flex items-center gap-1.5">
              ...
              {hotel.trips.filter(
                (t) =>
                  t.status === "open" && new Date(t.start_date) >= new Date(),
              ).length > 1 && (
          ...
          <div>
            <div>
              ...
              <p className="text-sm font-black text-orange-500">
                {nextTrip ? (
                  <>
                    {(nextTrip.price || 0).toLocaleString("vi-VN")}
    ...
    <div className="bg-white rounded-xl border border-gray-100 overflow-hidden text-sm">
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        ...
        {hasFilter && (
          <button
            onClick={onReset}
            className="text-xs text-orange-500 hover:underline bg-transparent border-none cursor-pointer"
          >
            Xóa tất cả
          </button>
        )}
...
function HotelListingContent() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch("https://db-pickyourway.vercel.app/api/tours").then((r) =>
        r.json(),
      ),
      fetch("https://db-pickyourway.vercel.app/api/trips/").then((r) =>
        r.json(),
      ),
      fetch("https://db-pickyourway.vercel.app/api/sales")
        .then((r) => r.json())
        .catch(() => ({ data: [] })),
      fetch("https://db-pickyourway.vercel.app/api/comments")
        .then((r) => r.json())
        .catch(() => ({ data: [] })),
      fetch("https://db-pickyourway.vercel.app/api/categories")
        .then((r) => r.json())
        .catch(() => ({ data: [] })),
    ])
      .then(([toursRes, tripsRes, salesRes, commentsRes, categoriesRes]) => {
        // thêm categoriesRes
        if (toursRes.success && Array.isArray(toursRes.data)) {
          const allTrips: TripAPI[] = Array.isArray(tripsRes.data)
            ? tripsRes.data
            : [];
          const ids = new Set<string>(
            (salesRes.data ?? []).map((s: any) => s.tour_id as string),
          );
          setSaleIds(ids);

          // Build commentMap: { tour_id -> { avg, count } }
          const allComments: { tour_id: string; rating: number }[] =
            commentsRes.data ?? [];
          const commentMap: Record<string, { avg: number; count: number }> = {};
          allComments.forEach((c) => {
            if (!c.tour_id || !c.rating) return;
            if (!commentMap[c.tour_id])
              commentMap[c.tour_id] = { avg: 0, count: 0 };
          ...
          setHotels(
            toursRes.data.map((t: TourAPI) =>
              mapTourToHotel(t, allTrips, commentMap, categoryMap),
            ),
          );
          console.log(
            "sample cities:",
            toursRes.data.slice(0, 5).map((t: any) => t.hotel_id?.city),
          );
        } else setError(true);
      })
      .catch(() => setError(true))
  ...
  const filtered = hotels.filter((h) => {
    ...
    if (searchName) {
      ...
      if (
        selectedRoute !== "Tất cả" &&
        !norm(h.city).includes(norm(selectedRoute))
      )
        return false;
      if (
        fromParam &&
        fromParam !== "Tất cả" &&
        !norm(h.city).includes(norm(fromParam))
      )
        return false;

      const q = norm(searchName);
      const regionCitiesForQ =
        Object.entries(REGION_MAP).find(([key]) => norm(key) === q)?.[1] ?? [];
      const hotelName = norm(h.name ?? "");
      const hotelCity = norm(h.city ?? "");
      const hotelTags = Array.isArray(h.tags) ? h.tags : [];
      if (regionCitiesForQ.length > 0) {
        if (
          !regionCitiesForQ.some((c) => {
            const nc = norm(c);
            return hotelCity.includes(nc) || nc.includes(hotelCity);
          })
        )
          return false;
        return false;
        ...
        const matchTag = hotelTags.some((t) => norm(t ?? "").includes(q));
    ...
    const ROUTE_ALIAS: Record<string, string[]> = {
      "tp.hcm": ["ho chi minh", "tphcm", "tp hcm", "hcm"],
      "ha noi": ["tp ha noi", "ha noi"],
      "da nang": ["da nang"],
      hue: ["hue"],
      "can tho": ["can tho"],
    };
    ...
    const normRoute = (s: string) =>
      s
        ...
        .toLowerCase()
        .trim();
    console.log("normRoute TP.HCM:", normRoute("TP.HCM"));
    console.log("normRoute TP. Hồ Chí Minh:", normRoute("TP. Hồ Chí Minh"));
    const routeMatches = (hotelRoute: string, filterRoute: string) => {
      const nr = normRoute(hotelRoute);
      const nf = normRoute(filterRoute);
      if (nr.includes(nf) || nf.includes(nr)) return true;
      // check alias
      const aliases = ROUTE_ALIAS[nf] ?? [];
      return aliases.some((a) => nr.includes(a));
    };

    if (selectedRoute !== "Tất cả" && !routeMatches(h.route, selectedRoute))
      return false;
    if (
      fromParam &&
      fromParam !== "Tất cả" &&
      !routeMatches(h.route, fromParam)
    )
      return false;

    if (
      selectedRoute !== "Tất cả" &&
      !normRoute(h.route).includes(normRoute(selectedRoute))
    )
      return false;
    if (
      fromParam &&
      fromParam !== "Tất cả" &&
      !normRoute(h.route).includes(normRoute(fromParam))
    )
      return false;
    ...
    if (regionParam) {
      ...
      const hotelCity = norm(h.city ?? "");
      const hotelName = norm(h.name ?? "");
      const matched = cities.some((c) => {
        const nc = norm(c);
        // "tp. ha noi".includes("ha noi") = true ✓
        return (
          hotelCity.includes(nc) ||
          nc.includes(hotelCity) ||
          hotelName.includes(nc)
        );
      });
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-1.5 flex items-center gap-1">
            <div
              ref={searchRef}
              className="relative flex items-center gap-2 flex-1 px-3 py-2 rounded-xl hover:bg-gray-50 transition-colors min-w-0"
            >
              <svg
              ...
              <div className="flex-1">
                ...
                <div className="relative w-28">
                  <input
                    ...
                    value={dateParam || ""}
                    className={`text-sm font-medium outline-none bg-transparent cursor-pointer w-full ${
                      dateParam ? "text-gray-700" : "text-transparent"
                    }`}
                    onChange={(e) => {
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400 mb-4">
          ...
          {searchName && (
            <>
              <span>/</span>
              <span className="text-gray-600 font-medium">"{searchName}"</span>
            </>
          )}
          {regionParam && !searchName && (
            <>
              <span>/</span>
              <span className="text-gray-600 font-medium">{regionParam}</span>
            </>
          )}
