...
import {
  Plane, MapPin, Users, Shield, ChevronRight,
...
function validate(name: string, value: string): string {
  switch (name) {
    ...
    case "phone": {
      ...
      if (!/^0\d{9}$/.test(clean)) return "SĐT phải 10 số và bắt đầu bằng 0 (VD: 0912345678)";
      return "";
    }
    case "address":
      if (!value.trim()) return "Vui lòng nhập địa chỉ";
      if (value.trim().length < 10) return "Địa chỉ quá ngắn (tối thiểu 10 ký tự)";
      return "";
    default:
      return "";
  }
...
export default function CheckoutPage() {
  ...
  const handleNext = () => {
    ...
    const newErrors  = Object.fromEntries(FIELDS.map((f) => [f, validate(f, (form as Record<string,string>)[f])]));
  ...
  const FieldIcon = ({ name }: { name: string }) => {
    ...
    return errors[name]
      ...
      : <CheckCircle2 size={15} className="text-green-400 absolute right-3 top-3" />;
  };
      ...
      <header className="bg-white border-b border-slate-100 shadow-sm sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div className="ml-auto flex items-center gap-2 text-sm">
            <span className="bg-indigo-600 text-white px-3 py-1 rounded-full font-medium text-xs">1 Thông tin</span>
            <ChevronRight size={14} className="text-slate-300" />
            <span className="text-slate-400 px-3 py-1 rounded-full text-xs border border-slate-200">2 Thanh toán</span>
            <ChevronRight size={14} className="text-slate-300" />
      ...
      <div className="max-w-6xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <div className="flex items-center gap-2 mb-5">
              ...
              <h2 className="font-semibold text-slate-700 text-lg">Chi tiết chuyến đi</h2>
            </div>
            <div className="flex gap-5 items-start bg-gradient-to-r from-indigo-50 to-blue-50 rounded-xl p-4 border border-indigo-100">
              {thumbnail ? (
                <img src={thumbnail} alt={tourName} className="w-28 h-20 rounded-xl object-cover shadow shrink-0" />
              ) : (
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-slate-800 text-base mb-2 leading-snug">{tourName}</h3>
                <div className="space-y-1.5 text-sm text-slate-500">
                  {hotelName && <div className="flex items-center gap-2"><span>🏨</span>{hotelName}</div>}
                  {city && <div className="flex items-center gap-2"><MapPin size={13} className="text-indigo-400" />{city}</div>}
          ...
          {/* Thông tin liên hệ */}
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
            <h2 className="font-semibold text-slate-700 text-lg mb-5">Thông tin liên hệ</h2>
            <div className="space-y-4">

              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Họ và tên <span className="text-red-400">*</span></label>
                <div className="relative">
                  <input name="name" value={form.name} onChange={handleChange} onBlur={handleBlur}
                    placeholder="Nguyễn Văn A" className={inputClass("name")} />
                  <FieldIcon name="name" />
                </div>
                {touched.name && errors.name && <p className="mt-1 text-xs text-red-500 flex items-center gap-1"><AlertCircle size={11} />{errors.name}</p>}
              </div>
              ...
              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Số điện thoại <span className="text-red-400">*</span></label>
                <div className="relative">
                ...
                {touched.phone && errors.phone
                  ? <p className="mt-1 text-xs text-red-500 flex items-center gap-1"><AlertCircle size={11} />{errors.phone}</p>
                  : !touched.phone && <p className="mt-1 text-xs text-slate-400">Nhập số bắt đầu bằng 0, đủ 10 chữ số</p>
                }
              </div>

              <div>
                <label className="block text-sm font-medium text-slate-600 mb-1.5">Địa chỉ <span className="text-red-400">*</span></label>
                <div className="relative">
                  <input name="address" value={form.address} onChange={handleChange} onBlur={handleBlur}
                    placeholder="123 Đường ABC, Quận X, TP.HCM" className={inputClass("address")} />
                  <FieldIcon name="address" />
                </div>
                {touched.address && errors.address
                  ? <p className="mt-1 text-xs text-red-500 flex items-center gap-1"><AlertCircle size={11} />{errors.address}</p>
                  : !touched.address && <p className="mt-1 text-xs text-slate-400">Nhập đầy đủ địa chỉ, tối thiểu 10 ký tự</p>
                }
              </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sticky top-24">
            <h2 className="font-semibold text-slate-700 text-lg mb-5">Tổng kết đơn hàng</h2>
            ...
            <div className="border-t border-slate-100 pt-4 mb-6">
              <div className="flex justify-between items-center">
                ...
                <span className="text-2xl font-bold text-indigo-600">{formatVND(total)}</span>
              </div>
            </div>
            ...
            {Object.values(errors).some(Boolean) && Object.values(touched).some(Boolean) && (
              <div className="mt-3 bg-red-50 border border-red-100 rounded-xl px-4 py-3 flex items-start gap-2">
                ...
                <p className="text-xs text-red-500">Vui lòng kiểm tra lại thông tin liên hệ.</p>
              </div>
            )}
