...
import { CheckCircle2, MapPin, Users, Shield, Home, Phone, Mail, ArrowLeft } from "lucide-react";
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 py-10 px-4">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-800 mb-1">Đặt tour thành công!</h1>
          <p className="text-slate-500 text-sm">Cảm ơn bạn đã tin tưởng TourViet</p>
          <div className="mt-4 inline-block bg-slate-50 border border-slate-200 rounded-xl px-5 py-2">
            <span className="text-xs text-slate-400">Mã đơn hàng</span>
            <p className="text-lg font-bold text-indigo-600 tracking-widest">{orderId}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
          {thumbnail && (
            <div className="relative h-44 overflow-hidden">
              ...
              <div className="absolute bottom-4 left-4 text-white">
                <p className="font-bold text-lg leading-tight">{tourName}</p>
                <p className="text-sm opacity-80 flex items-center gap-1">
                  <MapPin size={12} /> {hotelName} · {city}
                </p>
              </div>
            </div>
          )}
          <div className="p-5 space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-slate-500 flex items-center gap-1"><Users size={14} /> Khách</span>
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-slate-700">{PAYMENT_LABELS[payment] ?? payment}</span>
            </div>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
          <h2 className="font-semibold text-slate-700 mb-3">Thông tin liên hệ</h2>
        ...
        {/* QR MoMo nếu chọn MoMo */}
        {payment === "momo" && (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5 flex flex-col items-center gap-3">
            <h2 className="font-semibold text-slate-700 self-start">Thanh toán qua MoMo</h2>
            <p className="text-sm text-slate-400 self-start">Mở app MoMo → Quét mã bên dưới để hoàn tất</p>
            <div className="border-2 border-pink-200 rounded-2xl p-2 bg-white shadow-sm w-full">
              <img
                ...
                alt="MoMo QR Code"
                className="w-72 h-72 object-contain rounded-xl mx-auto"
              />
            </div>
            <p className="text-xs text-slate-400 text-center">Số tiền: <span className="font-bold text-pink-600">{formatVND(total)}</span></p>
          </div>
        )}

        {/* Chuyển khoản nếu chọn */}
        {payment === "transfer" && (
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 p-5">
            <h2 className="font-semibold text-slate-700 mb-3">Thông tin chuyển khoản</h2>
            <div className="bg-slate-50 rounded-xl p-4 space-y-2 text-sm text-slate-600 border border-slate-200">
              {[
        ...
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 text-sm text-amber-700">
          ...
          <ul className="list-disc list-inside space-y-1 text-xs text-amber-600">
            ...
            <li>Chúng tôi sẽ gửi xác nhận về email sau khi nhận được thanh toán</li>
        ...
        <div className="flex gap-3">
          <button
            onClick={() => router.push("/")}
            className="flex-1 flex items-center justify-center gap-2 bg-white border border-slate-200 text-slate-600 font-medium py-3 rounded-2xl hover:bg-slate-50 transition text-sm"
          >
            <Home size={16} />
            Về trang chủ
          </button>
          <button
            onClick={() => router.back()}
            className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 text-white font-medium py-3 rounded-2xl hover:bg-indigo-700 transition text-sm"
          >
            <ArrowLeft size={16} />
            Xem lại đơn hàng
          </button>
        ...
        <p className="text-center text-xs text-slate-400 flex items-center justify-center gap-1 pb-4">
          <Shield size={12} className="text-green-400" />
          Giao dịch được bảo mật bởi TourViet
        </p>
